...
function JobPanel({ data, role, onJob, overview = false }) {
  ...
  return <Panel id="home-jobs-title" title={overview ? 'Job overview' : 'Needs attention'} note={overview ? 'Required documents and recorded progress' : 'Overdue documents and NCR reports'} count={rows.length} tone={overview ? 'overview' : 'attention'}
    ...
    footer={<><span>Showing {Math.min(3, rows.length)} of {rows.length}</span><a href={filter === 'ncr' ? register('ncr') : monitor(filter === 'overdue' ? 'overdue' : '')}>{rows.length > 3 ? 'See more' : filter === 'ncr' ? 'Open NCR reports' : 'Open monitoring'}<IconChevronR size={14} /></a></>}>
    {rows.length ? <ul className="home-records">{rows.slice(0, 3).map(({ job, progress, late, ncr, target }) => <li key={job.jobNo}>
