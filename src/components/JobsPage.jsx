            ...
            <tbody>
              {shown.map((job, i) => {
                ...
                return (
                  ...
                  <tr className="jobs-clickable-row" onClick={() => navigate(`/job/${job.jobNo}`)}
                    title={`Open job ${job.jobNo}`}>
                    <td>
                      <a className="mon-primary jobs-job-link" href={`#/job/${job.jobNo}`}
                        onClick={(e) => e.stopPropagation()}>Job {job.jobNo}</a>
                    ...
                    <td className="jobs-status">
                      <div className="jobs-status-inner">
                        <span className={`chip chip-${st.id}`}>
                          {(() => { const G = STATUS_ICONS[st.id]; return <G size={13} /> })()}
                          {st.label}
                        </span>
                        <IconChevronR className="jobs-open-icon" size={15} aria-hidden="true" />
                      </div>
                    </td>
                  </tr>
                  </Fragment>
