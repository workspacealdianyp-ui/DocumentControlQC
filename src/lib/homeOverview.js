...
export function homeOverview(jobs, reports, ctx, name, now = new Date()) {
  ...
  for (const job of jobs) {
    ...
    if (progress.overdue || ncr) attention.push({ job, progress, late, pending, ncr, target: validTarget(job) })
  }
  ...
  const mine = reports.filter((r) => r.inspector === name && ['returned', 'draft'].includes(r.status))
  ...
  return {
    totals, attention, mine, submitted, counts,
